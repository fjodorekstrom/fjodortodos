$(document).ready(function(){
	...
	$("#btn-createTodo").click(function(){
		var todo_item = {"todo_item": [
				{
				name: $("#name").val().toString(),
				description: $("#description").val().toString()
				}
			]};
		var dataString = JSON.stringify(todo_item);
		$.ajax({
			type: "POST",
			...
			data: todo_item,
			success: function(){
				console.log("successfully posted: " + dataString);
				var a = todo_item["todo_item"];
				console.log(typeof a[0].name);
			}
		});
	});

	$("#btn-removeTodos").click(function(){
